...
class BridgeAccessTokenRepositoryTest extends TestCase
{
    ...
    public function test_access_tokens_can_be_persisted()
    {
        ...
        $tokenRepository = Mockery::mock('Andatech\Passport\TokenRepository');
        ...
        $accessToken = new Andatech\Passport\Bridge\AccessToken(2, [new Andatech\Passport\Bridge\Scope('scopes')]);
        $accessToken->setIdentifier(1);
        $accessToken->setExpiryDateTime($expiration);
        $accessToken->setClient(new Andatech\Passport\Bridge\Client('client-id', 'name', 'redirect'));

        $repository = new Andatech\Passport\Bridge\AccessTokenRepository($tokenRepository, $events);
