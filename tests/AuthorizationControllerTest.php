...
class AuthorizationControllerTest extends TestCase
{
    ...
    {
        Andatech\Passport\Passport::tokensCan([
            'scope-1' => 'description',
        ]);
        ...
        $controller = new Andatech\Passport\Http\Controllers\AuthorizationController($server, $response);
        ...
        $authRequest->shouldReceive('getScopes')->andReturn([new Andatech\Passport\Bridge\Scope('scope-1')]);
        ...
        $clients = Mockery::mock('Andatech\Passport\ClientRepository');
        $clients->shouldReceive('find')->with(1)->andReturn('client');

        $tokens = Mockery::mock('Andatech\Passport\TokenRepository');
    ...
    public function test_request_is_approved_if_valid_token_exists()
    {
        ...
        $authRequest->shouldReceive('getScopes')->once()->andReturn([new Andatech\Passport\Bridge\Scope('scope-1')]);
        ...
        $tokens = Mockery::mock('Andatech\Passport\TokenRepository');
        $tokens->shouldReceive('findValidToken')->with($user, 'client')->andReturn($token = Mockery::mock('Andatech\Passport\Token'));
